...
export function mongoList<T>(
  ...
  return apiRequest<
    Record<string, T[]> & { total: number; page: number; limit: number }
  >(`/api/mongodb/${resource}${suffix}`);
}

export function mongoCreate<T>(resource: string, body: unknown) {
  return apiRequest<{ item: T }>(`/api/mongodb/${resource}`, {
    method: "POST",
...
export function mongoUpdate<T>(resource: string, id: string, body: unknown) {
  return apiRequest<{ item: T }>(`/api/mongodb/${resource}/${id}`, {
    method: "PUT",
    body: JSON.stringify(body),
  });
}
...
export function mongoDelete(resource: string, id: string) {
  return apiRequest<{ ok: true }>(`/api/mongodb/${resource}/${id}`, {
    method: "DELETE",
  });
}
