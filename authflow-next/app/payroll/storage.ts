import type { PayrollDraft, PayrollEmployee, PayrollRecord, PayslipRecord } from "./types";
...
export function defaultPayrollDraft(employeeId = employees[0].id, month = new Date().toISOString().slice(0, 7)): PayrollDraft {
  const draft = baseDrafts[employeeId] || baseDrafts[employees[0].id];
  return { ...draft, employeeId, month };
}

export function mongoUsersToPayrollEmployees(users: MongoUser[], branches: MongoBranch[]): PayrollEmployee[] {
...
export function mongoPayrollToRecord(row: MongoPayroll, employee: PayrollEmployee, existingRecords: PayrollRecord[] = []): PayrollRecord {
  const workingDays = Number(row.workingDays || 0);
  const presentDays = Number(row.presentDays || 0);
  const grossSalary = Number(row.grossSalary ?? row.basicSalary ?? row.salary ?? 0);
  ...
  return {
    ...
    attendance: {
      ...
      attendancePercentage: Number(row.attendancePercentage ?? percentage(presentDays, workingDays)),
    },
    earnings: {
    ...
    ytdNetSalary: existingRecords
      .filter((record) => record.employeeId === employee.id && record.month.slice(0, 4) === row.month.slice(0, 4))
...
export function recordsToPayslips(records: PayrollRecord[]): PayslipRecord[] {
  return records.map((record) => ({ ...toPayslip(record), status: "saved" as const, savedAt: record.createdAt }));
}
