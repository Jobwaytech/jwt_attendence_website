...
const emptyLeave = {
  leaveType: "casual" as StudentLeave["leaveType"],
  fromDate: "",
  toDate: "",
  reason: "",
};
const emptyRegularization = {
  type: "missing_clock_in" as RegularizationType,
  date: "",
  requestedClockIn: "",
  requestedClockOut: "",
  reason: "",
...
export default function StudentPortal({ route }: { route: StudentRoute }) {
  ...
  useEffect(() => {
    ...
    if (route === "login" && session?.role === "student")
      window.location.replace("/student-dashboard");
  }, [loading, route, session]);

  const student = workspace.student || session;
  const summary = useMemo(() => {
    const present = workspace.attendances.filter(
      (item) => item.status === "present",
    ).length;
    const absent = workspace.attendances.filter(
    ...
    return {
      ...
      pendingTasks: workspace.tasks.filter(
        (item) => item.status !== "completed",
      ).length,
      completedTasks: completed,
      taskCompletion: percent(completed, workspace.tasks.length),
      leaveRequests: workspace.leaves.length,
      upcomingEvents: workspace.calendars.filter(
        (item) => item.startDate >= today(),
      ).length,
      faceVerified: workspace.attendances.filter((item) => item.faceVerified)
        .length,
      gpsVerified: workspace.attendances.filter((item) => item.gpsVerified)
        .length,
    };
  }, [workspace]);
  ...
  async function handleLogin(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!result.token || !result.user || result.user.role !== "student")
      ...
      toast(
        caught instanceof Error ? caught.message : "Student login failed.",
        true,
      );
    }
  }
  ...
  async function saveFaceRegistration() {
    ...
    try {
      ...
      toast(
        caught instanceof Error ? caught.message : "Unable to register face.",
        true,
      );
    } finally {
      setVerificationBusy(false);
  ...
  async function applyLeave(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    try {
      await apiRequest("/api/student/leaves", {
        method: "POST",
      ...
      toast(
        caught instanceof Error ? caught.message : "Unable to submit leave.",
        true,
      );
    }
  }
  ...
  async function submitRegularization(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    try {
      await apiRequest("/api/student/regularizations", {
        method: "POST",
  ...
  async function updateTask(
    ...
    try {
      await apiRequest(
        `/api/student/tasks/${task.id}/assignments/${task.assignmentId}`,
        {
          method: "PUT",
          body: JSON.stringify({
            status,
  ...
  function exportReport(format: "pdf" | "excel") {
    ...
    const csv = rows
      ...
      .join("\n");
    downloadBlob(
      `student-report-${student.studentId}.${format === "pdf" ? "pdf" : "csv"}`,
      csv,
  ...
  if (route === "login") {
    return (
      <main className="auth-page">
        <section
          className="auth-visual student-visual"
          aria-label="Student portal overview"
        >
          ...
          <div className="welcome-copy">
            <span className="eyebrow">
              <ShieldCheck /> Student access
            </span>
            <h1>Track attendance, tasks, leaves, reports, and schedules.</h1>
            <p>
              A dedicated student workspace with JWT, RBAC, and MongoDB data.
            </p>
          </div>
          <div className="floating-grid">
            <article>
              ...
              <span>Face, GPS, liveness</span>
            </article>
            <article>
              <ClipboardList />
              <strong>Tasks</strong>
              <span>Daily and internship work</span>
            </article>
            <article>
              <CalendarDays />
        ...
        <section className="auth-card">
          <div className="section-title">
            ...
            <div>
              <h2>Student login</h2>
              <p>Role is fixed to student for this portal.</p>
            </div>
          </div>
    ...
    <main className="app-shell">
      <aside className={`sidebar ${menuOpen ? "open" : ""}`}>
        ...
        <div className="sidebar-card">
          ...
          <span>JWT and RBAC protect student-only pages.</span>
        </div>
      </aside>
      ...
      <section className="content">
        <header className="topbar">
          ...
          <div className="profile-strip">
            <div className="avatar">{initials(displayName(student.name))}</div>
            <div>
              <strong>{displayName(student.name)}</strong>
        ...
        {workspaceLoading ? (
          <p className="demo-note">Loading MongoDB student workspace...</p>
        ) : null}
        {route === "dashboard" ? (
          <Dashboard
            student={student}
            ...
            attendance={workspace.attendances}
            events={workspace.calendars}
          />
        ) : null}
        {route === "profile" ? (
          <Profile student={student} faceProfile={workspace.faceProfile} />
        ) : null}
        {route === "attendance" ? (
          <AttendancePage
            cameraOn={cameraOn}
            ...
            clock={clock}
            summary={summary}
            records={workspace.attendances}
            faceSamples={faceSamples}
            captureFaceRegistrationSample={captureFaceRegistrationSample}
            saveFaceRegistration={saveFaceRegistration}
            faceProfile={workspace.faceProfile}
            verificationBusy={verificationBusy}
            livenessPrompt={livenessPrompt}
            regularizations={workspace.regularizations}
            regularizationForm={regularizationForm}
            setRegularizationForm={setRegularizationForm}
            submitRegularization={submitRegularization}
          />
        ) : null}
        {route === "tasks" ? (
          <TasksPage tasks={workspace.tasks} updateTask={updateTask} />
        ) : null}
        {route === "leaves" ? (
          <LeavesPage
            leaves={workspace.leaves}
        ...
        {route === "reports" ? (
          <ReportsPage summary={summary} exportReport={exportReport} />
        ) : null}
        {route === "calendar" ? (
          <CalendarPage events={workspace.calendars} />
        ) : null}
      </section>
      {notice ? (
        <div className={`toast ${error === notice ? "error" : ""}`}>
...
function Dashboard({
  ...
  const recent = [
    attendance[0]
      ? `Attendance ${attendance[0].status || "present"} on ${attendance[0].date}`
      : "No attendance activity yet",
    tasks[0]
      ? `${tasks[0].title} is ${tasks[0].status.replace("_", " ")}`
      : "No tasks assigned yet",
    leaves[0]
      ? `${leaves[0].leaveType} leave is ${leaves[0].status}`
  ...
  return (
    <section className="welcome-dashboard">
      <div className="hero-panel">
        <div>
          <span className="eyebrow">
            <FileBarChart /> Student dashboard
          </span>
          <h1>Welcome, {firstName(student.name)}.</h1>
          <p>
            Keep attendance, submissions, leaves, reports, and upcoming events
            in view.
          </p>
        </div>
        <div className="system-card">
          ...
          <span>{student.profile || "Student profile"}</span>
        </div>
      </div>
      ...
      <section className="panel">
        <div className="section-heading">
          <div>
            ...
            <p>
              Attendance progress, quick actions, recent activities, and
              upcoming events.
            </p>
          </div>
        </div>
        <div className="student-dashboard-grid">
          <article className="system-card">
            ...
            <div className="task-progress">
              <span style={{ width: `${summary.attendancePercentage}%` }} />
            </div>
            <span>
              {summary.present} of {summary.workingDays} attendance records
              present
            </span>
          </article>
          <article className="system-card">
    ...
    <section className="panel">
      <div className="section-heading">
        <div>
          ...
          <p>MongoDB student profile and face registration status.</p>
        </div>
      </div>
      <div className="profile-detail-grid">
        {Object.entries({
          ...
          Branch: student.branchId || "Not assigned",
          DOB: student.dob || "Not set",
          "Face Status": faceProfile.registered
            ? "Registered"
            : "Not registered",
          "Last Registration": faceProfile.updatedAt
            ? formatDateTime(faceProfile.updatedAt)
            : "Not registered",
        }).map(([key, value]) => (
          <article className="system-card" key={key}>
            <span>{key}</span>
            <strong>{value}</strong>
          </article>
        ))}
...
function AttendancePage(props: {
  ...
  startCamera: () => void;
  stopCamera: () => void;
  clock: (type: "clockin" | "clockout") => void;
  summary: Record<string, number>;
  records: AttendanceRecord[];
  faceSamples: FaceSample[];
  captureFaceRegistrationSample: () => void;
  saveFaceRegistration: () => void;
  faceProfile: FaceStatus;
  verificationBusy: boolean;
  livenessPrompt: string;
  regularizations: Regularization[];
  regularizationForm: typeof emptyRegularization;
  setRegularizationForm: (value: typeof emptyRegularization) => void;
  submitRegularization: (event: FormEvent<HTMLFormElement>) => void;
}) {
  return (
    <section className="panel">
      <div className="section-heading">
        <div>
          ...
          <p>
            Face enrollment, face verification, liveness, GPS, and attendance
            regularization.
          </p>
        </div>
      </div>
      <div className="stats-grid">
        <Stat
          label="Attendance"
          value={`${props.summary.attendancePercentage}%`}
          icon={<CalendarCheck />}
        />
        <Stat
          label="Present Days"
        ...
        <Stat
          label="Face Verified"
          value={props.summary.faceVerified}
          icon={<Camera />}
        />
        <Stat
          label="GPS Verified"
          value={props.summary.gpsVerified}
          icon={<FileBarChart />}
        />
      </div>
      <div className="attendance-console">
        ...
        <div className="hero-actions">
          {!props.cameraOn ? (
            ...
            <button className="ghost-button" onClick={props.stopCamera}>
              Stop camera
            </button>
          )}
          <button
            ...
            disabled={
              !props.cameraOn ||
              props.verificationBusy ||
              props.faceSamples.length >= 10
            }
            onClick={props.captureFaceRegistrationSample}
          >
            Capture face sample
          </button>
          <button
            className="ghost-button"
            disabled={props.verificationBusy || props.faceSamples.length < 3}
            onClick={props.saveFaceRegistration}
          >
            {props.faceProfile.registered
              ? "Re-register Face"
              : "Register Face"}
          </button>
          <button
            className="primary-button compact"
            disabled={!props.cameraOn || props.verificationBusy}
            onClick={() => props.clock("clockin")}
          >
            Clock In
          </button>
          <button
        ...
        <div className="branch-metrics">
          <span>
            Face status:{" "}
            {props.faceProfile.registered
              ? `Registered (${props.faceProfile.samples} samples)`
              : "Not registered"}
          </span>
          <span>{props.faceSamples.length}/10 new samples</span>
          <span>
            {props.livenessPrompt ||
              "Liveness prompt appears during attendance"}
          </span>
        </div>
      </div>
      <form className="inline-form" onSubmit={props.submitRegularization}>
        <select
          value={props.regularizationForm.type}
          onChange={(event) =>
            props.setRegularizationForm({
              ...props.regularizationForm,
        ...
        <input
          type="date"
          value={props.regularizationForm.date}
          onChange={(event) =>
            props.setRegularizationForm({
              ...props.regularizationForm,
        ...
        <input
          type="time"
          value={props.regularizationForm.requestedClockIn}
          onChange={(event) =>
            props.setRegularizationForm({
              ...props.regularizationForm,
        ...
        <input
          type="time"
          value={props.regularizationForm.requestedClockOut}
          onChange={(event) =>
            props.setRegularizationForm({
              ...props.regularizationForm,
        ...
        />
        <input
          value={props.regularizationForm.reason}
          onChange={(event) =>
            props.setRegularizationForm({
              ...props.regularizationForm,
      ...
      <StudentTable
        rows={props.records.map((item) => [
          "Attendance",
          item.date,
          formatDateTime(item.clockInAt),
          formatDateTime(item.clockOutAt),
          item.status || "present",
          `Face ${item.matchScore || 0}%`,
          item.gpsVerified ? "GPS Verified" : "GPS Pending",
          item.livenessVerified ? "Liveness Passed" : "Liveness Pending",
          item.invalidReason || item.securityWarning || "Verified record",
        ])}
        empty="No attendance history yet."
      />
      <StudentTable
        rows={props.regularizations.map((item) => [
          "Regularization",
...
function TasksPage({
  ...
  tasks: StudentTask[];
  updateTask: (task: StudentTask, status: TaskStatus, remarks: string) => void;
}) {
  const [drafts, setDrafts] = useState<
    Record<string, { status: TaskStatus; remarks: string }>
  >({});
  return (
    <section className="panel">
      <div className="section-heading">
        <div>
          <h1>Student tasks</h1>
          <p>
            Daily assignments, internship tasks, submissions, remarks, and
            deadlines.
          </p>
        </div>
      </div>
      <div className="task-grid">
        {tasks.map((task) => {
          const draft = drafts[task.assignmentId] || {
            status: task.status,
          ...
          return (
            <article className="task-card" key={task.assignmentId}>
              ...
              <p>
                {task.description || task.remarks || "No description added."}
              </p>
              <div className="task-controls">
                ...
                <button
                  className="primary-button compact"
                  onClick={() => updateTask(task, draft.status, draft.remarks)}
                >
                  Update
                </button>
              </div>
      ...
      {!tasks.length ? (
        <StudentTable rows={[]} empty="No tasks assigned yet." />
      ) : null}
    </section>
  );
}
...
function LeavesPage({
  ...
  leaves: StudentLeave[];
  leaveForm: typeof emptyLeave;
  setLeaveForm: (value: typeof emptyLeave) => void;
  applyLeave: (event: FormEvent<HTMLFormElement>) => void;
}) {
  return (
    <section className="panel">
      <div className="section-heading">
        <div>
          ...
          <p>
            Casual leave, sick leave, permission requests, and approval status.
          </p>
        </div>
      </div>
      <form className="inline-form" onSubmit={applyLeave}>
        <select
          ...
          onChange={(event) =>
            setLeaveForm({
              ...leaveForm,
              leaveType: event.target.value as StudentLeave["leaveType"],
            })
          }
        >
    ...
    <section className="panel">
      <div className="section-heading">
        <div>
          ...
          <p>
            Attendance and task completion summaries with PDF and Excel export.
          </p>
        </div>
        <div className="report-actions">
          <button className="ghost-button" onClick={() => exportReport("pdf")}>
            <Download /> Export PDF
          </button>
          <button
            className="ghost-button"
    ...
    <section className="panel">
      <div className="section-heading">
        <div>
          ...
          <p>
            Holidays, exams, company events, and training schedules from
            MongoDB.
          </p>
        </div>
      </div>
      <div className="calendar-grid">
        {events.map((event) => (
          <article className="calendar-card" key={event._id || event.id}>
            <div className="task-card-head">
              <div>
      ...
      {!events.length ? (
        <StudentTable rows={[]} empty="No calendar events yet." />
      ) : null}
    </section>
  );
}
  ...
  return (
    <div className="table-list">
      {rows.length ? (
        rows.map((row, index) => (
          <article
            className="table-row student-table-row"
            key={`${row[0]}-${index}`}
          >
